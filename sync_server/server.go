...
func StartSyncServer() {
	// 创建一个服务器
	Server, err := net.Listen("tcp", config.SyncServerAddress)

	if err != nil {
		log.Println("net.Listen err =", err)
		return
	}
	log.Println("start server success, listing on " + config.SyncServerAddress)
	defer Server.Close()
	// 接受文件名
	for {
		...
		// 拿到了文件的名字
		// create path. each time process one req,
		// means that each req only process create dir or create file
